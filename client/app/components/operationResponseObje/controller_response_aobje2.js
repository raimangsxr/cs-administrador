...
angular.module('csAdministratorApp')
  .controller('OperationResponseAOBJE2Ctrl', ['$rootScope', '$scope', '$log', '$http', '$cookies', '$uibModal', '$uibModalInstance', '$q', 'file', function ($rootScope, $scope, $log, $http, $cookies, $uibModal, $uibModalInstance, $q, file) {
        ...
        $http.get(file.link).then(
          function (response) {
            ...
            file_lines.forEach(function(line){
              ...
              obje_detail.cups = fields[1];
              obje_detail.fechaInicio = fields[2];
              obje_detail.fechaFin = fields[3];
              obje_detail.motivo = fields[4];
              obje_detail.publicado = parseInt(fields[5]);
              obje_detail.propuesto = parseInt(fields[6]);
              obje_detail.comentario = fields[7];
              obje_detail.objeAAutoObje = fields[8];
